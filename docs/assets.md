...
The stage follows the selected dark or light theme. Static is generated by project code, and the live Doom framebuffer is mapped onto the glass. `scripts/prepare-doom-assets.mjs` copies only the CC0 Belweder into `public/models/belweder.glb`; other package models are not shipped. Camera and placement calibration were adapted from CrazyGL's Apache-2.0 component. The older `prepare-tv-assets.mjs` package path repair is retained for reproducing the blank preview.

## Cursor cat

`public/oneko/oneko.gif` is the classic 256×128 sprite sheet from [adryd325/oneko.js](https://github.com/adryd325/oneko.js), served locally and unchanged (3,316 bytes). The cursor-following behavior is adapted into a lifecycle-managed React component. The MIT license and source/hash details are retained in [public/oneko](../public/oneko/README.md). The interaction was requested using [ramx.in](https://ramx.in/) as a reference; its custom avatar artwork is not used.

## Ambient television video

`public/videos/cofounder-2-hero.mp4` is the unchanged clip supplied by Rajdeep following the [Cofounder reference](https://cofounder.co/): 1920×1122 H.264, 24 fps, 2.375 seconds, 209,752 bytes, no audio track. A 960-pixel-wide WebP first frame is used while the video loads and when motion is paused. Both use the same slightly right-biased 4:3 cover crop, keeping the laptop visible without letterboxing or stretching. See [video provenance](../public/videos/README.md). This clip is a third-party reference asset, not project-created or CC0 artwork.
